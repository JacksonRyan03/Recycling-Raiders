...
this.sound.play();window.top.location.href='https://www.vu.edu.au/';}}});EntityButtonStart=EntityButton.extend({size:{x:181,y:60},animSheet:animSheetList.buttonStart[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled){this.sound.play();}
...
this.sound.play();window.top.location.href='https://www.cleanup.org.au/';}}});EntityButtonBack=EntityButton.extend({size:{x:186,y:58},animSheet:animSheetList.buttonBack[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
...
this.sound.play();}}});EntityButtonContinueHowTo=EntityButton.extend({size:{x:132,y:43},animSheet:animSheetList.buttonContinue[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
this.sound.play();ig.system.setGame(HowToScreen);}}});EntityButtonContinueGame=EntityButton.extend({size:{x:132,y:43},animSheet:animSheetList.buttonContinue[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
this.sound.play();ig.system.setGame(RogersGame);}}});EntityButtonRestart=EntityButton.extend({size:{x:186,y:58},animSheet:animSheetList.buttonRestart[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
...
this.tickCount=Math.ceil(this.score/10000);if(this.secondsTimer.delta()>0){ig.game.seconds++;this.secondsTimer.reset();for(var i=0;i<this.tickCount;i++){if(!this.entityItemTable.length){this.newItemTable();}
...
ig.music.add('media/audio/theme-music.*');ig.music.volume=0.3;if(ig.Sound.enabled){ig.music.play();this.BtnAudio.currentAnim=this.BtnAudio.anims['true'];}
else{this.BtnAudio.currentAnim=this.BtnAudio.anims['false'];}
...
this.counter++;this.parallax.move(40);this.parent();},draw:function(){this.parent();this.parallax.draw();this.BtnStart.draw();this.BtnEnclave.draw();this.BtnBlackmoon.draw();this.BtnFacebook.draw();this.BtnAudio.draw();var range=((this.counter/3)%60)-30;var delta=(range>0)?1:-1;var tick=range*delta;this.ship.draw(-30,ig.system.height-this.ship.height+tick+10);this.logo.draw((ig.system.width-this.logo.width)/2,10);this.title.draw((ig.system.width-this.title.width+160)/2,100);this.bestScore.draw(10,ig.system.height-10-50);this.font.draw(this.oldHighscore,70,ig.system.height-28,ig.Font.ALIGN.CENTER);}});StoryScreen=ig.Game.extend({story:animSheetList.screenStory[_LANG_CODE],buttonContinue:animSheetList.buttonContinueImg[_LANG_CODE],init:function(){ig.input.bind(ig.KEY.SPACE,'start');ig.input.bind(ig.KEY.ENTER,'start');this.BtnContinue=this.spawnEntity(EntityButtonContinueHowTo,ig.system.width-132-10,ig.system.height-43-10);},update:function(){if(ig.input.pressed('start')){ig.system.setGame(HowToScreen);}
if(!this.BtnContinue)
this.BtnContinue=this.spawnEntity(EntityButtonContinueHowTo,ig.system.width-132-10,ig.system.height-43-10);this.BtnContinue.update();this.parent();},draw:function(){this.parent();this.story.draw(0,0);if(this.BtnContinue)
this.BtnContinue.draw();}});HowToScreen=ig.Game.extend({howTo:animSheetList.screenHowto[_LANG_CODE],buttonContinue:animSheetList.buttonContinueImg[_LANG_CODE],init:function(){ig.input.bind(ig.KEY.SPACE,'start');ig.input.bind(ig.KEY.ENTER,'start');ig.input.bind(ig.KEY.H,'howTo');this.BtnContinue=this.spawnEntity(EntityButtonContinueGame,ig.system.width-132-10,ig.system.height-43-10);},update:function(){if(ig.input.pressed('howTo')){ig.system.setGame(StartScreen);}
if(ig.input.pressed('start')){ig.system.setGame(RogersGame);}
if(!this.BtnContinue)
this.BtnContinue=this.spawnEntity(EntityButtonContinueGame,ig.system.width-132-10,ig.system.height-43-10);this.BtnContinue.update();this.parent();},draw:function(){this.parent();this.howTo.draw(0,0);if(this.BtnContinue)
this.BtnContinue.draw();}});if(ig.ua.iOS){ig.Sound.enabled=false;}
